...
def categorize_tx(w3, *, chain_data, txs, api_url, api_key, tx_repo, account_address):
    try:
        for tx in txs:
            ...
            tx_type, send, recv = determine_tx_type(w3, tx, logs, tx_summary, api_url, api_key, account_address)
...
def determine_tx_type(w3, tx, logs, tx_summary, api_url, api_key, account_address):
    send = recv = ''
    tx_type = ''

    result = is_transfer_tx(w3, tx, logs, tx_summary, api_url, api_key, account_address)
    if result[0]:
...
def is_swap_tx(w3, tx_summary, api_url, api_key):
...
def is_transfer_tx(w3, tx, logs, tx_summary, api_url, api_key, account_address):
        ...
        for log in logs:
            ...
            if event_sig_hash == TRANSFER_EVENT_SIG_HASH:
                ...
                if w3.to_checksum_address('0x' + log['topics'][1].hex()[-40:]) == w3.to_checksum_address(
                    ...
                    tx_summary[log['address']] = {'from': True, 'amount': amount}
                ...
                if w3.to_checksum_address('0x' + log['topics'][2].hex()[-40:]) == w3.to_checksum_address(
                    ...
                    tx_summary[log['address']] = {'to': True, 'amount': amount}

            if event_sig_hash == DEPOSIT_EVENT_SIG_HASH:
                tx_summary[log['address']] = {'deposit': True, 'amount': amount}
            if event_sig_hash == WITHDRAWAL_EVENT_SIG_HASH:
                tx_summary[log['address']] = {'withdrawal': True, 'amount': amount}
    return tx_summary
