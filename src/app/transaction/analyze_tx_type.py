...
from src.app.contract.is_nft_contract import is_nft_contract
from src.app.repositories.tx_repo import TxRepo
from src.app.settings.si import TRANSFER_EVENT_SIG_HASH, DEPOSIT_EVENT_SIG_HASH, \
    WITHDRAWAL_EVENT_SIG_HASH, CHAIN_ID_PARAM
from src.app.transaction.process_bridge import process_bridge_tx, check_if_bridge_tx
from src.app.transaction.process_swap import process_swap_tx
from src.app.transaction.process_transfer import process_transfer_tx, check_if_transfer_tx
from src.app.transaction.save_tx import save_tx
from src.app.transaction.transform_tx_data import transform_tx_data
...
def categorize_tx(*, w3, txs, api_url, api_key, tx_repo, account_address, logger):
    try:
        for tx in txs:
            ...
            tx_summary = process_token_transfer_logs(w3=w3, logs=logs, api_url=api_url, api_key=api_key,
            ...
            if (is_nft_contract(w3=w3, api_url=api_url, api_key=api_key, address=tx['to'], logger=logger)
                ...
                continue
            tx_type, send, recv = determine_tx_type(w3=w3, tx=tx, logs=logs, tx_summary=tx_summary, api_url=api_url,
                                                    api_key=api_key, account_address=account_address, logger=logger)
            print(tx_summary, tx['nonce'], tx['hash'])
            if tx_type:
                save_tx(transform_tx_data(w3=w3, api_url=api_url, api_key=api_key, l1_fee=tx_receipt['l1Fee'],
                                          tx=tx, tx_type=tx_type, account_address=account_address, logger=logger,
