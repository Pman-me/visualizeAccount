...
def get_contract_name(*, api_url: str, api_key: str, contract_address, logger):
    try:
        res = json.loads(
            requests.get(f"{api_url}&module=contract&action=getsourcecode&address={contract_address}&apikey={api_key}")
            .text)
